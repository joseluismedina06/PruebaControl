    ...
    <?php
        ...
        if($idpartyenterprise!="" && $maintenancesignaling==""){
            $dbl=new baseBL();
            $com="SELECT maintenancesignaling FROM core.securityplancomplements WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $maintenancesignaling=$dbl->executeScalar($com);
        }    
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                if ($idsecurityplancomplements=="") {
                    $com="INSERT INTO core.securityplancomplements 
                        (idparty,maintenancesignaling)
                        VALUES ('$idpartyenterprise','$maintenancesignaling') RETURNING id";
                        var_dump($com);
                    $idsecurityplancomplements=$dbl->executeScalar($com);
                    
                    if ($idsecurityplancomplements>0 && $idsecurityplancomplements!="") {
                        $path = "nmcapacitacionPL.php";
                    ...
                    SET maintenancesignaling='$maintenancesignaling' WHERE idparty='$idpartyenterprise' RETURNING id";
                    var_dump($com);
                    $idsecurityplancomplements=$dbl->executeScalar($com);
        ...
        $registrobl->buildLinks("nmmantenimientoPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.nmmantenimientoPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
    ...
    ?>
        ...
        <body>    
            <FORM action="<?php //echo $action;?>" method="post" name="nmmantenimientoPL" class="italsis">
            <!--Navegacion-->
            <drawer-menu>
            <div align="center">
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.nmmantenimientoPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);
    
        presentationLayer::buildFormTitle('  
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>MANTENIMIENTO</div>
            ',"");     

        echo '<div align=center>';

        myPresentationLayer::buildTextAreasInpunClass($title, "maintenancesignaling", "maintenancesignaling", $maintenancesignaling,"input textarea","title", $rows, $cols);
        echo '</div>';       
