    ...
    <html>
        <head>
            ...
            <title>Programa de Difusión y Concientización</title>
    ...
    <?php
        ...
        $disseminationconsentprogram = basePL::getReq($_REQUEST,"disseminationconsentprogram");
        
        if($idpartyenterprise!="" && $disseminationconsentprogram==""){
            $dbl=new baseBL();
            $com="SELECT disseminationconsentprogram FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $disseminationconsentprogram=$dbl->executeScalar($com);
        }        
        ...
        if ($urloper == "save" && ($msj_error=="" && $error_msg_first_regist=="")) {
            ...
            // }else{
            if($msj_error=="" && $error_msg_first_regist==""){
                $com="SELECT id FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
                $iddisseminationconsentprogram=$dbl->executeScalar($com);
                // $oper = "insert";  
                if ($iddisseminationconsentprogram=="") {
                    $com="INSERT INTO core.securityplanextras 
                        (idparty,disseminationconsentprogram)
                        VALUES ('$idpartyenterprise','$disseminationconsentprogram') RETURNING id";
                        // var_dump($com);
                    $iddisseminationconsentprogram=$dbl->executeScalar($com);
                    
                    if ($iddisseminationconsentprogram>0 && $iddisseminationconsentprogram!="") {
                        $path = "nmsimulacrosPL.php";
                    ...
                    $com="UPDATE core.securityplanextras 
                    SET disseminationconsentprogram='$disseminationconsentprogram' WHERE idparty='$idpartyenterprise' RETURNING id";
                // var_dump($com);
                    $iddisseminationconsentprogram=$dbl->executeScalar($com);
        ...
        $registrobl->buildLinks("nmprogramadedifusionyconcientizacionPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.nmprogramadedifusionyconcientizacionPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.nmprogramadedifusionyconcientizacionPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);
    
        presentationLayer::buildFormTitle('  
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>PROGRAMA DE DIFUSION Y CONCIENTIZACION</div>
                ',"");     

        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "disseminationconsentprogram", "disseminationconsentprogram", $disseminationconsentprogram,"input textarea","title", $rows, $cols);
        echo '</div>';      
    
        echo '<div style="">
        <span style="color:black; font-size: x-large; ">Evidencia</span>
        </div>';
        echo '<br>';
        ...
        echo '<br>';    
        myPresentationLayer::inputFileImg("DIFUSION-0","DIFUSION-0","DIFUSION-0-".$idpartyenterprise,"DIFUSION","show-DIFUSION-0-","fa fa-images custom-file-label-icon", "","","img-previewiddocument img-fluid",
            "","","","","");
