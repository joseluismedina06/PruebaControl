    ...
    <html>
        <head>
            ...
            <title>Planes de Emergencia</title>
    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $emergencyplans  = basePL::getReq($_REQUEST,"emergencyplans");
        
        if($idpartyenterprise!="" && $emergencyplans==""){
            $dbl=new baseBL();
            $com="SELECT emergencyplans FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $emergencyplans=$dbl->executeScalar($com);
        }        
        ...
        if($idpartyenterprise==""){
            ...
            // if ($urloper == "save" && $emergencyplans=="") {
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                if ($idsecurityplansitelocation=="") {
                    $com="INSERT INTO core.securityplanextras 
                        (idparty,emergencyplans)
                        VALUES ('$idpartyenterprise','$emergencyplans') RETURNING id";
                        //var_dump($com);
                    $idsecurityplansitelocation=$dbl->executeScalar($com);
                    if ($idsecurityplansitelocation>0 && $idsecurityplansitelocation!="") {
                        $path = "nmevaluaciondedanosPL.php";
                    ...
                    SET emergencyplans='$emergencyplans' WHERE idparty='$idpartyenterprise' RETURNING id";
                    //var_dump($com);
                    $idsecurityplansitelocation=$dbl->executeScalar($com);
                    if ($idsecurityplansitelocation>0 && $idsecurityplansitelocation!="") {
                        $path = "nmevaluaciondedanosPL.php";
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.nmplanesdeemergenciaPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);

        presentationLayer::buildFormTitle('  
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>PLANES DE EMERGENCIA</div>
            ',"");     

        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "emergencyplans", "emergencyplans", $emergencyplans,"input textarea","title", $rows, $cols);
        echo '</div>';      
