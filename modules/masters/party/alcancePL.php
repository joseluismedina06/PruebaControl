    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $scope  = basePL::getReq($_REQUEST,"scope");
        
        



        if($idpartyenterprise!="" && $scope==""){
            $dbl=new baseBL();
        ...
        $registrobl->buildLinks("alcancePL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.alcancePL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
...
$dbl->executeScalar($com);
    ...
    ?>
        ...
        <body>    
            <FORM action="<?php echo $action;?>" method="post" name="alcancePL" class="italsis">
    		<!--Navegacion-->
            <drawer-menu>
            <div align="center">
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.alcancePL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);

        presentationLayer::buildFormTitle('  
    <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>ALCANCE</div>
           ',"");   
 
    echo '<div style="text-align: center;">

    <!--<span style="color:black; font-size: x-large; ">ALCANCE</span>-->
    </div>';  
    echo '<div align=center>';
    myPresentationLayer::buildTextAreasInpunClass($title, "scope", "scope", $scope,"input textarea","title", $rows, $cols);
    echo '</div>';
