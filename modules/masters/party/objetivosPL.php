    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $objetives  = basePL::getReq($_REQUEST,"objetives");
        
        
        if($idpartyenterprise!="" && $objetives==""){
            $dbl=new baseBL();
        ...
        if($idpartyenterprise==""){
            ...
            // if ($urloper == "save" && $objetives=="") {
            //      $error_msg_first_regist .= "El campo Objetivos no puede estar vacio. "; 
            // }
        }
        ...
        if ($urloper == "save" && ($msj_error=="" && $error_msg_first_regist=="")) {
            ...
            //consultar si existe la ci          
                ...
                // }else{
                    if($msj_error=="" && $error_msg_first_regist==""){
                        // $com="SELECT id FROM base.entitysubclass where code='PERSONINFOVALUES' and name='BASIC'";
                        // $idclinktype=$dbl->executeScalar($com);
                        // $com="Select core.isspinsclinkid(NULL,NULL,NULL,'$idclinktype',"
                        //     . "'$active','$deleted')";   
      
                        // $idclink=$dbl->executeScalar($com);
                        // $oper = "insert";  
                        // if ($idclink!="") {
                            // $date_create=date(DATE_ATOM);
                            // $com="SELECT core.isspinsclinkerid(NULL,NULL,'$idclink', NULL, '$email', '$password', '$date_create','$active', '$deleted')";
                            $com="UPDATE core.securityplansitelocation 
                            SET objetives='$objetives' WHERE idparty='$idpartyenterprise' RETURNING id";
                      // var_dump($com);
        ...
        $registrobl->buildLinks("objetivosPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.objetivosPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
...
$dbl->executeScalar($com);
    ...
    ?>
        ...
        <body>    
            <FORM action="<?php echo $action;?>" method="post" name="objetivosPL" class="italsis">
    		<!--Navegacion-->
            <drawer-menu>
            <div align="center">
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.objetivosPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);

        presentationLayer::buildFormTitle('  
    <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>OBJETIVOS</div>
           ',"");    

    echo '<div style="text-align: center;">

    <!--<span style="color:black; font-size: x-large; ">OBJETIVOS</span>-->
    </div>';  

    echo '<div align=center>';
    myPresentationLayer::buildTextAreasInpunClass($title, "objetives", "objetives", $objetives,"input textarea","title", $rows, $cols);
    echo '</div>';    
