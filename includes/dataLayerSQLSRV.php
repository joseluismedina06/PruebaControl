<?php
	include_once("sqldbITALSISSQLSRV.php");
	include_once("presentationLayer.php");
	

  	class dataLayerSQLSRV {
		...
		function open() {
			...
			$this->con = new sqldbITALSISSQLSRV($this->server,$this->user,$this->password,$this->database);
			//echo "antes de connect SQLSRV";
			$res = $this->con->sqlConnect($this->server,$this->user,$this->password,$this->database);
			//echo "despueses de connect SQLSRV";
