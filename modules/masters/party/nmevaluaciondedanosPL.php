    ...
    <html>
        <head>
            ...
            <title>Evaluación de Daños</title>
    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $damageevaluation  = basePL::getReq($_REQUEST,"damageevaluation");
        
        if($idpartyenterprise!="" && $damageevaluation==""){
            $dbl=new baseBL();
            $com="SELECT damageevaluation FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $damageevaluation=$dbl->executeScalar($com);
        }        
        ...
        if($idpartyenterprise==""){
            ...
            // if ($urloper == "save" && $damageevaluation=="") {
        ...
        if ($urloper == "save" && ($msj_error=="" && $error_msg_first_regist=="")) {
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                $idsecurityplanextras=$dbl->executeScalar($com);
                // $oper = "insert";  
                if ($idsecurityplanextras=="") {
                    $com="INSERT INTO core.securityplanextras 
                        (idparty,damageevaluation)
                        VALUES ('$idpartyenterprise','$damageevaluation') RETURNING id";
                        // var_dump($com);
                    $idsecurityplanextras=$dbl->executeScalar($com);
                    if ($idsecurityplanextras>0 && $idsecurityplanextras!="") {
                        $path = "nmsubprogramaderecuperacionPL.php";
                    ...
                    SET damageevaluation='$damageevaluation' WHERE idparty='$idpartyenterprise' RETURNING id";
                // var_dump($com);
                    $idsecurityplanextras=$dbl->executeScalar($com);
                    if ($idsecurityplanextras>0 && $idsecurityplanextras!="") {
                        $path = "nmsubprogramaderecuperacionPL.php";
        ...
        $registrobl->buildLinks("nmevaluaciondedanosPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.nmevaluaciondedanosPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.nmevaluaciondedanosPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);

        presentationLayer::buildFormTitle('  
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>EVALUACION DE DAÑOS</div>
            ',"");     

           
        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "damageevaluation", "damageevaluation", $damageevaluation,"input textarea","title", $rows, $cols);
        echo '</div>';      
