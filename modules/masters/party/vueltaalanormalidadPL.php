    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $cometonormal  = basePL::getReq($_REQUEST,"cometonormal");
        
        if($idpartyenterprise!="" && $urloper!="save"){
            $dbl=new baseBL();
            $com="SELECT cometonormal FROM core.securityplancomplements WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $cometonormal=$dbl->executeScalar($com);
        }        
             ...
             $dbl=new baseBL();
                    if($msj_error=="" && $error_msg_first_regist==""){
                        ...
                        if($res==""){
                          ...
                          (idparty,cometonormal)
                          VALUES ('$idpartyenterprise','$cometonormal') RETURNING id";
                            $idsecurityplancomplements=$dbl->executeScalar($com);
                            // var_dump($com);                            
                        }else{                        
                            $com="UPDATE core.securityplancomplements 
                            SET cometonormal='$cometonormal' WHERE idparty='$idpartyenterprise' RETURNING id";
                      // var_dump($com);
                            $idsecurityplancomplements=$dbl->executeScalar($com);
                            // var_dump($com);
                        }
        ...
        $registrobl->buildLinks("vueltaalanormalidadPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.vueltaalanormalidadPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
...
// nowcertifiesdate
        ...
        <body>    
            <FORM action="<?php echo $action;?>" method="post" name="vueltaalanormalidadPL" class="italsis">
    		<!--Navegacion-->
            <drawer-menu>
            <div align="center">
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.vueltaalanormalidadPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);

        presentationLayer::buildFormTitle('  
    <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>VUELTA A LA NORMALIDAD</div>
           ',"");       
...
echo '<div style="">
    <!--<span style="color:black; font-size: x-large; ">VUELTA A LA NORMALIDAD</span>-->
    </div>'; 
    echo '<div align="center">';        
    myPresentationLayer::buildTextAreasInpunClass($title, "cometonormal", "cometonormal", $cometonormal,"input textarea","title", $rows, $cols);
    echo '</div>';
    ...
    echo '<br>';
myPresentationLayer::inputFileImg("VUELNORMAL-0","VUELNORMAL-0","VUELNORMAL-0-".$idpartyenterprise,"VUELNORMAL","show-VUELNORMAL-0-","fa fa-images custom-file-label-icon", "","","img-previewiddocument img-fluid",
    "","","","","");

       
    $com="SELECT * from core.pictures where idparty='$idpartyenterprise'  and name like '%VUELNORMAL-0-%' and delete='N'";
    $imgs=$dbl->executeReader($com);  


echo '<div class="col-12 col-lg-8" id="show-VUELNORMAL-0-" style="margin-top:1rem; display: flow-root;">';
    $i=0;
    if($imgs!=""){
        foreach($imgs as $b){            
