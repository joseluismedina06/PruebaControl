    ...
    <?php
        ...
        $especificactivityprogram = basePL::getReq($_REQUEST,"especificactivityprogram");
        $compliancereport = basePL::getReq($_REQUEST,"compliancereport");
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                if ($idsecurityplanextras=="") {
                    $com="INSERT INTO core.securityplanextras 
                        (idparty,especificactivityprogram,compliancereport)
                        VALUES ('$idpartyenterprise','$especificactivityprogram','$compliancereport') RETURNING id";
                        //var_dump($com);
                    $idsecurityplanextras=$dbl->executeScalar($com);
                    /*
                    ...
                    SET especificactivityprogram='$especificactivityprogram',
        ...
        if($idpartyenterprise!=""){
            ...
            $dbl=new baseBL();
            $com="SELECT especificactivityprogram,compliancereport 
                FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $res=$dbl->executeReader($com);
            if($res!="") {
                $especificactivityprogram=$res[0]['especificactivityprogram'];
                $compliancereport=$res[0]['compliancereport'];
            }
        }
    ...
    <?php
        ...
        <i class="material-icons">menu</i></div>DOCUMENTACION</div>
            ',"");     

        echo '<div style="">
        <span style="color:black; font-size: x-large; ">Programa de actividades específicas</span>
        </div>';

        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass(
            $title, "especificactivityprogram", "especificactivityprogram", $especificactivityprogram,
            "input textarea","title", $rows, $cols);
        echo '</div>';       
    
        echo '<div style="">
        <span style="color:black; font-size: x-large; ">Informe de Cumplimiento</span>
        </div>';

        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "compliancereport", "compliancereport", 
        $compliancereport,"input textarea","title", $rows, $cols);
        echo '</div>'; 
