    ...
    <?php
        ...
        //----- var Datos basic
        //  
        $alerting  = basePL::getReq($_REQUEST,"alerting");
        
        if($idpartyenterprise!="" && $alerting==""){
            $dbl=new baseBL();
            $com="SELECT alerting FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $alerting=$dbl->executeScalar($com);
        }        
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                if ($idsecurityplanextras=="") {
                    $com="INSERT INTO core.securityplanextras 
                        (idparty,alerting)
                        VALUES ('$idpartyenterprise','$alerting') RETURNING id";
                        // var_dump($com);
                    $idsecurityplanextras=$dbl->executeScalar($com);
                    
                    if ($idsecurityplanextras>0 && $idsecurityplanextras!="") {
                        $path = "nmplanesdeemergenciaPL.php";
                    ...
                    SET alerting='$alerting' WHERE idparty='$idpartyenterprise' RETURNING id";
                    // var_dump($com);
                    $idsecurityplanextras=$dbl->executeScalar($com);
    ...
    <?php
        ...
        presentationLayer::buildIdInputHidden($id,"document.nmalertamientoPL",$fLink);
        myPresentationLayer::buildInputHidden("idparty","idparty","idparty",$idparty);
    
        presentationLayer::buildFormTitle('  
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>ALERTAMIENTO</div>
            ',"");     

        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "alerting", "alerting", $alerting,"input textarea","title", $rows, $cols);
        echo '</div>';      
