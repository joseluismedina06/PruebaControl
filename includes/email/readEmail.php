...
//$exchange_server = "https://GISRVEXC-fl.GrupoItalcambio.int"; 

$exchange_server = "https://192.168.206.10"; 

//$exchange_server = "https://mail.italcambio.com"; 

$exchange_username = "GRUPOITALCAMBIO.INT\MVAZQUEZ"; 
$exchange_password = "Vickita_*"; 
...
// find all the folders in the inbox for a user named 'mvazquez'. 
$h->xmlrequest = '<?xml version="1.0"?>'; 
$h->xmlrequest .= <<<END
<a:searchrequest xmlns:a="DAV:" xmlns:s="http://schemas.microsoft.com/exchange/security/"> 
   <a:sql> 
       SELECT "DAV:displayname" 
       FROM SCOPE('hierarchical traversal of "$exchange_server/Exchange/mvazquez/inbox"')  
   </a:sql> 
</a:searchrequest> 
END;
//FROM SCOPE('hierarchical traversal of "$exchange_server/Exchange/mvazquez/inbox"')  
...
if (!$h->fetch($exchange_server."/Exchange/mvazquez/inbox", 0, null, $exchange_username, $exchange_password, "SEARCH")) { 
  echo "<h2>There is a problem with the http request!</h2>"; 
  echo $h->log; 
  exit(); 
} 
//if (!$h->fetch($exchange_server."/Exchange/mvazquez2/inbox", 0, null, $exchange_username, $exchange_password, "SEARCH")) { 
