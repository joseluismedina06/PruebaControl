    ...
    <?php
        ...
        $training  = basePL::getReq($_REQUEST,"training");
        
        if($idpartyenterprise!="" && $training==""){
            $dbl=new baseBL();
            $com="SELECT training FROM core.securityplanextras WHERE idparty='$idpartyenterprise'";
            // var_dump($com);
            $training=$dbl->executeScalar($com);
        }        
        ...
        if ($urloper == "save" && ($msj_error=="" && $error_msg_first_regist=="")) {
            ...
            if($msj_error=="" && $error_msg_first_regist==""){
                ...
                $idtraining=$dbl->executeScalar($com);
                // $oper = "insert";  
                if ($idtraining=="") {
                    $com="INSERT INTO core.securityplanextras
                        (idparty,training)
                        VALUES ('$idpartyenterprise','$training') RETURNING id";
                        //var_dump($com);
                    $idtraining=$dbl->executeScalar($com);
                    
                    if ($idtraining>0 && $idtraining!="") {
                        $path = "nmprogramadedifusionyconcientizacionPL.php";
                    ...
                    SET training='$training' WHERE idparty='$idpartyenterprise' RETURNING id";
                    //var_dump($com);
                    $idtraining=$dbl->executeScalar($com);
        ...
        $registrobl->buildLinks("nmcapacitacionlPL.php",$pn,$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink,$action);    
        $bpl = new basePL("document.nmcapacitacionPL",$sLink,$dLink,$pLink,$cLink,$fLink,$fbnLink);
    ...
    ?>
        ...
        <body>    
            <FORM action="<?php //echo $action;?>" method="post" name="nmcapacitacionPL" class="italsis">
            <!--Navegacion-->
            <drawer-menu>
            <div align="center">
    ...
    <?php
        ...
        <div data-drawer-open><div style="float:left;"><i class="material-icons">menu</i></div>CAPACITACION</div>
            ',"");     

           
        echo '<div align=center>';       
        myPresentationLayer::buildTextAreasInpunClass($title, "training", "training", $training,"input textarea","title", $rows, $cols);
        echo '</div>';      
    
        echo '<div style="">
        <span style="color:black; font-size: x-large; ">Evidencia de la Capacitación</span>
        </div>';
        echo '<br>';
        ...
        echo '<br>';    
        myPresentationLayer::inputFileImg("TRAINING-0","TRAINING-0","TRAINING-0-".$idpartyenterprise,"TRAINING","show-TRAINING-0-","fa fa-images custom-file-label-icon", "","","img-previewiddocument img-fluid",
            "","","","","");
