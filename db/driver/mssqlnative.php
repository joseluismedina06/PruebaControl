...
include_once("mssqlbase.php");
include_once("../../errorCollector.php");

class mssqlnative extends \proteccioncivil\db\driver\mssqlbase
{
	...
	* {@inheritDoc}
	*/
	function sqlConnect($sqlserver, $sqluser, $sqlpassword, $database, $port = false, $persistency = false, $new_link = false)
	{
		...
		return ($this->db_connect_id) ? $this->db_connect_id : $this->sqlError('');
	}

	/**
	* {@inheritDoc}
	*/
	function sqlServerInfo($raw = false, $use_cache = true)
	{
		global $cache;
	...
	* {@inheritDoc}
	*/
	function sqlQuery($query = '', $cache_ttl = 0)
	{
		if ($query != '')
		{
			global $cache;
			...
			$this->sqlAddNumQueries($this->query_result);
	...
	* Build LIMIT query
	*/
	function psqlQueryLimit($query, $total, $offset = 0, $cache_ttl = 0)
	{
	...
	*/
	function sqlFetchrow($query_id = false)
	{
		...
		if ($cache && $cache->sqlExists($query_id))
		{
			return $cache->sqlFetchrow($query_id);
		}
	...
	* {@inheritDoc}
	*/
	function sqlFreeresult($query_id = false)
	{
		global $cache;
	...
	*/
	function mssqlnativeNumRows($res)
	{
		if ($res !== false)
		{
	...
	*/
	function mssqlnativeSetQueryOptions($options)
	{
		$this->query_options = $options;
	}
        
        //AGREGADO DE mssql.php
